...
class BmpLoader {
private:FILE* file;
		void getImageSize();//memeber function
...
BmpLoader::BmpLoader(const char* filename)//constructor for opening bmp file
{
	iWidth = 0, iHeight = 0;
...
void BmpLoader::getImageSize()//read the header of bmp file extract information about the image, such as width, height, and color depth.
{
...
BmpLoader::~BmpLoader()//The destructor deallocates the memory used for storing image data.
{
	delete[] data;
}
#endif BMPLOADER_H
